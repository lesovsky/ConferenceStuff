...
psql -f sql/08-archive-config.sql &>/dev/null && echo "${calm}OK${reset}" || echo "${fail}FAILED${reset}"
...
du -sh /var/lib/pgsql/${PG_VERSION}/data/pg_xlog/|awk '{print $2" "$1}'
...
pgbench -n -c 2 -T 60 -f sql/08-update-games.pgb pgbench &>/dev/null
...
read -p "${sand}Press Enter to run pgbench ${reset}"
pgbench -n -c 3 -T 60 -f sql/08-update-games.pgb pgbench &>/dev/null
