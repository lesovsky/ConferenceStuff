...
psql -f sql/09-check-games-buffercache.sql -U postgres pgbench_6gb

echo -n -e "${sand}Check test table's size and bloat\n${reset}"
psql -f sql/09-check-games-bloat.sql -U postgres pgbench_6gb
...
pgbench -n -c 8 -T 60 -f sql/09-update-games.pgb -U postgres pgbench_6gb
