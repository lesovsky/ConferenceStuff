...
psql -U postgres -f sql/12-checkpoints-bad-config.sql &>/dev/null && echo "${calm}OK${reset}" || echo "${fail}FAILED${reset}"
...
pgbench -U postgres -c 8 -T 180 -P 1 -R 5000 -f sql/12-checkpoints-pgbench-test.pgb pgbench_6gb

read -p "${sand}Show pg_stat_bgwriter stat.${reset}"
psql -U postgres -P pager=off -x -f sql/12-checkpoints-ckpt-bgwr-report.sql

read -p "${sand}Press any key to continue.${reset}"

echo -n -e "${sand}Configure better checkpoints... ${reset}"
psql -U postgres -f sql/12-checkpoints-better-config.sql &>/dev/null && echo "${calm}OK${reset}" || echo "${fail}FAILED${reset}"
echo -n -e "${calm}max_wal_size = 1200MB\ncheckpoint_timeout = 90s\ncheckpoint_completion_target = 0.9\n"
...
psql -U postgres -f sql/12-checkpoints-restore-settings.sql &>/dev/null && echo "${calm}OK${reset}" || echo "${fail}FAILED${reset}"
